...
def build_dashboard():
    ...
    ws_summary["A1"] = "naver_news_it 비즈니스 대시보드"
    ...
    # 언론사별 기사 수 요약 테이블 작성
    ws_summary["A7"] = "언론사별 기사 수 집계"
    ws_summary["A7"].font = Font(name="맑은 고딕", size=13, bold=True)
    ...
    for press, count in press_counts.items():
        ws_summary.cell(row=current_row, column=1, value=press).font = data_font
        ws_summary.cell(row=current_row, column=1).alignment = Alignment(horizontal="left")
        ws_summary.cell(row=current_row, column=1).border = thin_border
        
        ws_summary.cell(row=current_row, column=2, value=count).font = data_font
    ...
    # 합계 행 추가
    ws_summary.cell(row=current_row, column=1, value="합계").font = Font(name="맑은 고딕", size=10, bold=True)
    ws_summary.cell(row=current_row, column=1).alignment = Alignment(horizontal="center")
    ws_summary.cell(row=current_row, column=1).border = thin_border
    
    ws_summary.cell(row=current_row, column=2, value=f"=SUM(B9:B{current_row-1})").font = Font(name="맑은 고딕", size=10, bold=True)
    ws_summary.cell(row=current_row, column=2).alignment = Alignment(horizontal="right")
    ws_summary.cell(row=current_row, column=2).border = thin_border
