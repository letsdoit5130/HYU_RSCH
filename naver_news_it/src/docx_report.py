"""
naver_news_it Word 비즈니스 보고서 생성 스크립트
...
def build_docx_report():
    ...
    title_run = title.add_run("naver_news_it 종합 비즈니스 보고서")
    ...
    doc.add_paragraph(f"수집된 원시 데이터는 총 {len(df)}건이며, 주요 필드는 title, summary, publisher, link, date 등 입니다.")
    
    # 데이터 테이블 추가
    display_cols = ["title", "publisher", "summary", "date"]
    korean_headers = ["기사 제목", "언론사", "기사 요약", "수집일"]
    
    table = doc.add_table(rows=1, cols=len(display_cols))
    table.style = 'Table Grid' # 보다 표준적인 그리드 스타일 적용
    hdr_cells = table.rows[0].cells
    
    # 헤더 작성
    for idx, header_name in enumerate(korean_headers):
        hdr_cells[idx].text = header_name
        
    # 데이터 일부(상위 5개) 채우기
    for r_idx in range(min(len(df), 5)):
        row_cells = table.add_row().cells
        for c_idx, col_name in enumerate(display_cols):
            row_cells[c_idx].text = str(df.iloc[r_idx][col_name])
