...
7. **삼국무역(중계무역) 후보 매트릭스 (신규)**: HS Code별로, `--home_country` 소싱이 여의치 않을 때를 대비한
   ...
   시장에 이미 진출해 있는지(점유율)로 화이트스페이스 여부를 표시. 전체 HS Code를 통틀어 **① 최대
   마진갭 ② 화이트스페이스 최우선 ③ 대형 시장 안정성** 기준으로 우선 개척 조합 3선을 먼저 요약하고,
   그 아래 HS Code별 전체 후보 표를 제공. FTA 미적용/중계무역 신고 절차/제재 대상국 확인 필요성을
   항상 함께 명시.
8. **1인 상사 창업자 맞춤형 3대 시장개척 전략** (CAGR 기준으로 고성장 시장 산출). 시장 3곳마다
   "이 시장이 실제로 가장 많이 수입하는 HS Code TOP 2"를 **추천 품목**으로 함께 표시해, "어느
   품목을 어느 시장에 팔아야 하는지"까지 답합니다.
