...
7. **삼국무역(중계무역) 후보 매트릭스 (신규)**: HS Code별로, `--home_country` 소싱이 여의치 않을 때를 대비한
   ...
   시장에 이미 진출해 있는지(점유율)로 화이트스페이스 여부를 표시. 조합마다 **권장 매입가**(B 평균단가
   − 표준편차 ~ 평균, 협상 목표 밴드), **권장 판매가**(C/D 평균단가 ~ 평균 + 표준편차, 오퍼 밴드),
   **B측 컨택 파트너**(템플릿, 실사 확인 필요 명시)를 함께 제시. 전체 HS Code를 통틀어 **① 최대
