"""
kyobooks_harness 탐색적 데이터 분석 (EDA) 및 마크다운 리포트 생성 스크립트
...
def run_analysis():
    ...
    # 마크다운 리포트 빌드
    report_content = []
    report_content.append(f"# kyobooks_harness 탐색적 데이터 분석 (EDA) 보고서\n\n")
    report_content.append("## 1. 데이터 기본 개요\n")
    report_content.append(f"- **전체 데이터 행 수**: {df.shape[0]}건\n")
    report_content.append(f"- **컬럼 구성**: 상품번호, 순위, 도서명, 저자, 출판사, 출판일, 정가, 할인가, 할인율, 평점\n\n")
