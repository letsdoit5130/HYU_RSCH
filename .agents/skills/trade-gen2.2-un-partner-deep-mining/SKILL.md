...
3. **스코프 지정 모드에 진입했다면, 개수를 명시했는지 여부와 무관하게 진행 중 사용자에게
   "계속 진행할까요?", "더 찾아볼까요?" 같은 중간 확인 질문을 하지 않는다.** 표N/특정
   국가/"최대한 많이" 중 무엇으로 스코프가 지정됐든 동일하게 적용된다 — 시장조사가 목적이므로
   개수를 말하지 않았다는 것이 "적당히 하다 그만두라"는 뜻이 아니라 "가능한 만큼 다 찾으라"는
   뜻이다. 종료 조건은 둘 중 먼저 오는 것이다: (a) 사용자가 개수를 명시했다면 그 개수 도달,
   (b) 개수를 명시하지 않았다면 검색 각도를 다 소진했는데도 더 이상 출처 URL로 검증 가능한
   후보가 나오지 않는 시점. 그 전까지는 국가/쿼리 각도를 바꿔가며 스스로 계속 반복한다.
   중간 경과 보고는 생략하고, 6단계(최종 보고)에서만 몇 개국·몇 건을 조사했는지, (개수가
   있었다면) 목표에 도달했는지(못 했다면 이유 — 후보 소진 또는 검증 가능한 결과 없음)를 한
   번에 보고한다. 기본 모드(범위 미지정, 상위 3~5개국·국가당 2~3개 쿼리)는 이 규칙과 무관하게
   기존 동작을 유지한다.
4. 각 finding의 `scope` 필드에 어떤 표/HS Code를 대상으로 조사했는지 기록한다
...
**이메일을 못 찾았다고 바로 빈칸 처리하지 않는다.** 첫 페이지(주로 홈)에 이메일이 안 보이면,
같은 도메인에서 흔히 쓰이는 연락처 페이지를 1~2개 더 시도한다: `/contact`, `/contact-us`,
`/inquiry`, `/en/contact`, `/en/inquiry`, `/about` 순으로. 그래도 없으면 그때 빈칸 + "이메일
미기재, 문의폼만 존재" 등 note를 남긴다. (실제 사례: 홈페이지엔 이메일이 없었지만
`/en/inquiry/` 페이지에는 있었던 경우 — 첫 페이지만 보고 포기하면 이런 걸 놓친다.)

**WebFetch가 열리지 않거나(접속 오류) 페이지 내용이 그 회사와 안 맞으면, 그 URL을 그대로
`source_url`로 쓰지 않는다.** `"{정확한 회사명}" official site` 식으로 한 번 더 검색해서 실제로
열리는 올바른 URL을 찾은 뒤 그걸로 대체한다 — 검색 결과에 뜬 링크라고 무조건 진짜 홈페이지라고
가정하지 않는다 (오래된 인덱스, 이전 도메인 등으로 죽은 링크가 섞여 있을 수 있음).

**LinkedIn 등 회사 자체 홈페이지가 아닌 곳에서 회사를 발견했다면, 거기서 검증을 끝내지 않는다.**
회사 자체 웹사이트 URL이 그 페이지에 언급돼 있거나 검색으로 추정 가능하면, **그 웹사이트도
반드시 별도로 WebFetch해서** (a) 실제로 접속되는지, (b) 연락처/문의 페이지에 이메일이 있는지
확인한다. "LinkedIn 페이지가 열렸으니 출처 URL 요건은 충족했다"며 회사 자체 사이트를 안 열어보고
넘어가지 않는다 (실제 사례: Godak Marketing Corp.를 LinkedIn 기업 페이지로만 확인하고
`godak.co.jp`는 열어보지 않아 이메일도 못 찾고, 실제 접속 여부도 확인 안 된 웹사이트 URL만
기록된 채 끝난 경우 — 이런 걸 방지하기 위함). 웹사이트 URL을 `website` 칸에 적으려면 그 URL을
실제로 WebFetch해서 열린다는 것까지 확인한 뒤에만 적는다 — 접속을 확인 못 한 URL은 빈칸 두고
note에 "웹사이트 URL 미확인(추정)"이라고 남긴다.
